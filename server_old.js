...
function addClosingDateInfo(job) {
  ...
  return {
    ...job,
    closingDate: job.closingDate || closingDate,
    closingStatus: job.closingStatus || getClosingStatus(job.closingDate || closingDate)
  };
}
...
app.post("/auto-search", async (req, res) => {
  try {
    ...
    const userQuery = query || "resident medical officer RMO jobs Australia hospital doctor";
...
app.post("/score-jobs", async (req, res) => {
  try {
    const { jobs, quickProfile } = req.body;

    if (!Array.isArray(jobs) || jobs.length === 0) {
      return res.status(400).json({ error: "No jobs received for scoring." });
    }

    const splitJobs = splitAndDedupeJobs(jobs, quickProfile);
    const jobsToScore = await enrichTopJobsWithFirecrawl(splitJobs.suitable.slice(0, 20));
    const cacheKey = `score:${SCORING_PROVIDER}:${JSON.stringify(jobsToScore.map(j => j.link))}:${quickProfile}`;
...
Applicant:
${quickProfile || "No profile provided"}
...
`;
    ...
    const text = await askGeminiCached(cacheKey, prompt, MODEL_SMART);
    const nameMatch = String(profile || "").match(/Name:\s*(.*)/);
    const levelMatch = String(profile || "").match(/Level:\s*(.*)/);
